...
class ControleModelo(Modelo,DataInput):
    ...
    def save_list(self,type_save:str)->None:
        if type_save == 'donwload':self.download_list_download(self.table['save'])
        else :self.save(type_save=type_save,data_table=self.table['save'])


    def get_conteudo_do_arquivo(self,type_arq:str, Arquivo:Arquivo):
        if type_arq=='pdf': return self.get_pdf_arq_content(Arquivo=Arquivo)
